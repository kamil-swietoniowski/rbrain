...
pub fn command_line(path: &str) -> rusqlite::Result<()> {
    ...
    loop {
        ...
        let commands: Vec<String> = buffer.split_whitespace().map(|s| s.to_string()).collect();
        ...
        match commands[0].as_str() {
            ...
            "modify" => {
                ...
                match commands[1].as_str() {
                    "title" => modify_record_title(
                        &db,
                        commands.get(2).cloned(),
                        commands.get(3).cloned(),
                    )?,
                    "content" => modify_record_content(
                        &db,
                        commands.get(2).cloned(),
                        commands.get(3).cloned(),
                    )?,
            ...
            "tag" => {
                ...
                match commands[1].as_str() {
                    "add" => insert_tag_into_record(
                        &db,
                        commands.get(2).cloned(),
                        commands.get(3).cloned(),
                    )?,
                    "rm" => remove_tag_from_record(
                        &db,
                        commands.get(2).cloned(),
                        commands.get(3).cloned(),
                    )?,
            ...
            "help" => {
                println!("Avaible commands: \n");
                println!("add [title] [content]");
                println!("rm [id]");
                println!("show [id]");
                println!("ls");
                println!("modify title [id] [title]");
                println!("modify content [id] [title]");
                println!("search tag add [id] [tag]");
                println!("search tag rm [id] [tag]");
                println!("exit");
            }
...
fn add_to_database(
    ...
    let mut title = title;
    let mut content = content.unwrap_or("".to_string());
    if title.is_none() {
    ...
    let content = if input("Encrypt? (y/n)").to_lowercase() == "y" {
        let password = input("Password: ");
        Some(encrypt::encrypt(content, &password))
    } else {
        Some(content)
    };
...
fn show_record(db: &Database, id: Option<String>) -> rusqlite::Result<()> {
    ...
    let mut record = db.get_record_from_database(id)?;

    if input("Decrypt? (y/n): ").to_lowercase() == "y" {
        let password = input("Input password: ");
        record.content = Some(encrypt::decrypt(
            record.content.unwrap_or("".to_string()),
            &password,
        ));
    }
